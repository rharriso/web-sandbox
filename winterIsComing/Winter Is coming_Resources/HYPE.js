var HYPE = (function HYPE() {
	...
	this.addHypeBuiltWatermark = function () {
		var contentContainer = parent.document.getElementById(this.mainContentContainerID);
	...
	this.overlayLoadingPage = function () {
		var loadingDiv = parent.document.getElementById(this.loadingPageID());
		if(loadingDiv == null) {	
			var contentContainer = parent.document.getElementById(this.mainContentContainerID);
	...
	this.addAdditionalCSS = function () {
		var additionalCSS = ".HYPE_scene {\
			...
			@-webkit-keyframes HYPE_swapToFront {\
				0% {\
					-webkit-transform: translate3d(-256px, 0px, -600px) rotateY(50deg);\
					-webkit-animation-timing-function: ease-in;\
				}\
				50% {\
					-webkit-transform: translate3d(-512px, 0px, -300px) rotateY(40deg);\
			...
			@-webkit-keyframes HYPE_swapToBack {\
				...
				50% {\
					-webkit-transform: translate3d(512px, 0px, -300px) rotateY(-20deg);\
					-webkit-animation-timing-function: ease-out;\
				}\
			\
				100% {\
	...
	this.showScene = function (sceneNumber, transition) {
		...
		var currentSceneContainer = parent.document.getElementById(this.currentSceneIdentifier());
	...
	this.showSceneWithInstant = function (sceneNumber) {
		...
		var nextSceneContainer = parent.document.getElementById(this.currentSceneIdentifier());
	...
	this.showSceneWithCrossfade = function (sceneNumber) {
		...
		if(currentSceneContainer != null) {
			...
			for(var key in initialValues) {
				...
				parent.document.getElementById(key).style["-webkit-box-reflect"] = null;
			}
		}
	...
	this.showSceneWithSwap = function (sceneNumber) {
		...
		parent.document.getElementById(this.mainContentContainerID).style["-webkit-perspective"] = "600px";
	...
	this.identifierOfSceneAtIndex = function (sceneIndex) {
		return this.scenes[sceneIndex].oid;
	}
	...
	this.loadScene = function (sceneIdentifier) {
		...
		var scene = parent.document.getElementById(sceneIdentifier);
		...
		for(var handlerName in keyHandlerNames) {
			...
			if(this.scenes[sceneIndex][handlerName] != null) {
				...
				var keyFunction = this.Apply.CreateActionHandler(this, scenes[sceneIndex][handlerName], document);
				var hypeContext = this;
	...
	this.completeSceneTransition = function (currentSceneContainer, nextSceneContainer) {
		...
		parent.document.getElementById(this.mainContentContainerID).style["-webkit-perspective"] = null;
	...
	this.stopAllTimelineRuns = function () {
		...
		var scene = parent.document.getElementById(this.currentSceneIdentifier());
		var sceneIndex = scene.getAttribute("HYPE_scene_index");
		var elements = getElementsByClassName("HYPE_element", scene);
	...
	this.timelineRunCompleteCallback = function (timelineRun) {
		...
		for(var i = 0; i < timelineRun.activeAnimations.length; i++) {
			var element = parent.document.getElementById(timelineRun.activeAnimations[i]["oid"]);
		...
		for(var i = timelineRun.nextAnimationIndex; i < animations.length; i++) {
			...
			if(currentTime >= startTime) {
				if(animation["type"] == this.kAnimationTypeStandardVideo) {
					...
					var element = parent.document.getElementById(oid);
	...
	this.removeCSS3Transition = function (animation) {
		var element = parent.document.getElementById(animation["oid"]);
		if(element == null) {
			return;
		}
	...
	this.Apply = ({
			...
			TextShadowYOffset : function (hypeDoc, value, element) {
				...
				hypeDoc.Apply.TetShadow(element);
			},
